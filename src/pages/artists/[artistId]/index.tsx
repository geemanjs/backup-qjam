...
import {ListItem, ListItemLeft, ListItemTitle} from "../../../components/ListItem";
...
function seoConfig(artist: Artist) {
  return {
    ...
    description: `Get the QJAM app to live video chat with ${
      artist.displayName
    } and receive a digital signing.`,
    openGraph: {
      title: `Meet ${artist.displayName} on QJAM`,
      description: `Get the QJAM app to live video chat with ${
        artist.displayName
...
export const ArtistView = ({ liveEvents, upcomingEvents, archivedEvents, artist }: IProps) => {
      ...
      />
        <Container px={0}>
          ...
          >
            ...
            {/*/>*/}
            <EventImageBottom>
              <Flex justifyContent="space-between" alignItems="center" width={1} px={3} py={2}>
          ...
          {artist.isUnclaimed && (
            <Text
              ...
              px={2}
            >
              <Text fontWeight="bold">Want to live video chat with {artist.displayName}?</Text>
              As a valued fan, the power is in your hands to bring {artist.displayName} to QJAM.
              <br />
              <br />
          ...
          <React.Fragment>
            <Container pb={[4, 4, 2]}>
              <div style={{ position: 'relative' }}>
                <Heading textAlign={["left", "left", "center"]} pb={3} fontSize="md">
                  FEATURED MUSIC
                </Heading>
                <Heading textAlign={["left", "left", "center"]} pb={3} fontSize="md">
                  {artist.music.title}
                  ...
                  {artist.spotifyUri && (
                    <Flex mr={10} flexDirection="column" alignItems="center">
                      <Link href={artist.spotifyUri} passHref={true}>
                        <Button as="a" bg="teal.400" color="white" borderRadius="lg">View on Spotify</Button>
                      </Link>
                    </Flex>
                  )}
                  {artist.appleMusicUrl && (
                    <Flex flexDirection="column" alignItems="center" mt={[10, null, 0]}>
                      <Link href={artist.appleMusicUrl} passHref={true}>
                        <Button as="a" bg="teal.400" color="white" borderRadius="lg">View on Apple Music</Button>
                      </Link>
                    </Flex>
                  )}
                </ViewOnMusicServiceWrapper>
            ...
            <Box bg="white" pt={0} pb={3} p={0}>
              {artist.music.tracks.map((song: MusicTrack, index) => (
                <Box _even={{ backgroundColor: "rgba(0, 0, 0, 0.1)" }} _hover={{ backgroundColor: "#49b8ab", color: "#fff"}} key={index} song={song} />
              ))}
            </Box>
          </React.Fragment>
        )}
...
export const getStaticProps: GetStaticProps<IProps, {artistId: string}> = async (context) => {
  if(!context.params) {
