...
class Artists {
  ...
  parseArtist = (artist: Artist) => {
    ...
    if (artist.coverPhoto) {
      artist.coverPhoto.loc = `${process.env.NEXT_PUBLIC_CLOUDINARY_IMAGE_API}/${artist.coverPhoto.loc}.jpg`;
    } else {
      artist.coverPhoto = {
        loc: `/img/icons/no-cover-photo.png`,
        type: "",
      };
    }
    if (artist.profilePic) {
      artist.profilePic.loc = `${process.env.NEXT_PUBLIC_CLOUDINARY_IMAGE_API}/${artist.profilePic.loc}.jpg`;
    } else {
      artist.profilePic = { loc: `/img/icons/no-avatar.png`, type: "" };
    }
    return artist;
  };
}
